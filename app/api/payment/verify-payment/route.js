...
export async function POST(req) {
  try {
    ...
    const paymentData = existingDoc.data();
    const existingStatus = paymentData.status;
    ...
    if (existingStatus !== finalStatus) {
      ...
      console.log(`[Verify Payment] Updated payment status to ${finalStatus} for order ${order_id}`);
    }

    // --- 4. If payment is successful, update user subscription ---
    ...
    // --- 5. Return the Updated Record to the Frontend ---
