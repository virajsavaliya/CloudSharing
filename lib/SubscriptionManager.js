...
const sendEmail = async (to, subject, html) => {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL_UPGRADE || process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
    // Remove trailing slashes and /f/ to get clean base URL
    const cleanBaseUrl = baseUrl.replace(/\/$/, '').replace(/\/f\/$/, '');
    
    const response = await fetch(`${cleanBaseUrl}/api/send-email`, {
      method: 'POST',
      headers: {
  ...
  static async sendExpirationReminders() {
    try {
      ...
      for (const doc of expiringSubscriptions.docs) {
        ...
        const upgradeUrl = process.env.NEXT_PUBLIC_BASE_URL_UPGRADE || process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
        const cleanUpgradeUrl = upgradeUrl.replace(/\/$/, '').replace(/\/f\/$/, '');
        ...
        const emailBody = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${cleanUpgradeUrl}/upgrade"
